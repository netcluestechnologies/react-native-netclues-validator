export class Validator {
    ...
    static emailValidate(email: string, reGex: RegExp = /^[^\s@]+@[^\s@]+\.[^\s@]+$/): boolean {
        return reGex.test(email);
    }

    static phoneNumberValidate(phone: string, reGex: RegExp = /^\+?[1-9]\d{1,14}$/): boolean {
        return reGex.test(phone);
    }

    static strongPasswordValidate = (password: string, reGex: RegExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/): boolean => {
        return reGex.test(password);
    };

    static creditCardNumberValidate = (cardNumber: string, reGex: RegExp | string = ''): boolean => {
        ...
        const isValidFormat = Object.values(CARD_FORMATS).some(regex => regex.test(formattedCardNumber));

        return isValidFormat;
    }


    static cvvValidate(cvv: string, cardType: string): boolean {
